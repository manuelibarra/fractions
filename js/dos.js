var mapaDos={
    create: function() {
        ...
        if(dosmov){
            for(var p=0;p<=dospos;p++){
                game.add.image(640+p*15, 10, 'block');
            }
            game.add.text(800, 10, ((dospos+1)*10)+"%", stylelevel);
        }else{
            for(var p=0;p<dospos;p++){
                game.add.image(640+p*15, 10, 'block');
            }
            game.add.text(800, 10, (dospos*10)+"%", stylelevel);
        }
        game.add.image(640, 10, 'progress'); //añadimos barra progreso
        
        //niño
        this.kid = game.add.sprite(26+dospos*81, 460, 'kid7'); //cargamos sprite niño
        ...
        if(dosmov){ //Si hay que mover al niño
            ini = 26+dospos*81; //posicion inicial
            this.fin = ini+81; //posicion final
        }
        if(!dosmov){ //si es la primera vez que se ingresa al menu
            this.cuenta = 0;
            this.espera = 120;
        }
    ...
    update: function() {
        ...
        if(dosmov){ //si hay que mover a uno
    ...
    cargaNivel: function(){
        ...
        if(dospos<10){
            game.state.start('unodos');
        }else{
            game.state.start('dosfinal');
        }
    },
...
var unoDos={
    create: function() {  
        ...
        game.add.text(700, 10, "Nivel : "+(dospos+1), style);
        
        //variables de control
        this.clicked = false; //posicionar globo
        this.convierte = false; //convertir circulos a medios
        ...
        this.circles = game.add.group(); //arreglo de circulos
        this.labels = game.add.group(); //arreglo de mensajes (medios)
        this.numit = game.rnd.integerInRange(1, 6); //número de medios
        ...
        for(var p=0;p<this.numit;p++){
            ...
            //circulos tapando con label
            var circle = game.add.graphics(60, 485-p*80);
            circle.anchor.setTo(0.5,0.5);
            circle.lineStyle(2, 0x31314e);
            circle.beginFill(0xefeff5);
            circle.drawCircle(0, 0, 80);            
            circle.endFill();
            this.circles.add(circle);

            var medio = game.add.sprite(60, 485-p*80, 'umid');
            medio.anchor.setTo(0.5,0.5);
            this.labels.add(medio);
        }
    ...
    update: function() {
        ...
        //posiciona
        if(this.convierte){
            if(this.labels.x>=74){  //Si las etiquetas pasaron el limite
                if(this.circles.alpha>0){ //si todavia tiene opacidad
                    this.circles.alpha-=0.01;   //reducimos
                }else{  //si llegó a cero, ocultamos
                    this.circles.visible = false;
                    this.labels.visible = false;
                }
            }else{ //sino aumentamos lugar x
                this.labels.x +=1;
            }
            
            if(!this.circles.visible){ //si ya se han escondido los circulos
                this.anima = true;
